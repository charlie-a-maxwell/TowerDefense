                           ...
                           D3DXMATERIAL* pd3dxMaterials, DWORD dwMaterials )
{
    ...
    rgdwAdjacency = DX_NEW DWORD[pInMesh->GetNumFaces() * 3];
...
HRESULT CDXUTMesh::CreateMaterials( LPCWSTR strPath, IDirect3DDevice9 *pd3dDevice, D3DXMATERIAL* d3dxMtrls, DWORD dwNumMaterials )
{
    ...
    if( d3dxMtrls && m_dwNumMaterials > 0 )
    {
        ...
        m_pMaterials = DX_NEW D3DMATERIAL9[m_dwNumMaterials];
        if( m_pMaterials == NULL )
            return E_OUTOFMEMORY;
        m_pTextures = DX_NEW LPDIRECT3DBASETEXTURE9[m_dwNumMaterials];
        if( m_pTextures == NULL )
            return E_OUTOFMEMORY;
        m_strMaterials = DX_NEW CHAR[m_dwNumMaterials][MAX_PATH];
                                  ...
                                  bool bSplitVertexForOptimalTangents )
{
    ...
    if( pTempMesh )
    {
        ...
        if( bHaveNormalNow && !bHadTangent && bHaveTangentNow && bAutoComputeTangents )
        {
            ...
            DWORD *rgdwAdjacency = NULL;
            rgdwAdjacency = DX_NEW DWORD[m_pMesh->GetNumFaces() * 3];
            if( rgdwAdjacency == NULL )
                return E_OUTOFMEMORY;
                             ...
                             CDXUTMeshFrame* pParentFrame )
{
    ...
    if( Guid == TID_D3DRMFrame )
    {
        ...
        // Create the frame
        pCurrentFrame = DX_NEW CDXUTMeshFrame( strName );
                            ...
                            CDXUTMeshFrame* pParentFrame )
{
    ...
    // Create the mesh
    pParentFrame->m_pMesh = DX_NEW CDXUTMesh( strName );
