...
HRESULT DXUTSetDevice( IDirect3DDevice9* pd3dDevice )
{
    ...
    DXUTDeviceSettings* pDeviceSettings = DX_NEW DXUTDeviceSettings;
...
HRESULT DXUTChangeDevice( DXUTDeviceSettings* pNewDeviceSettings, IDirect3DDevice9* pd3dDeviceFromApp, bool bForceRecreate, bool bClipWindowToSingleAdapter )
{
    ...
    DXUTDeviceSettings* pNewDeviceSettingsOnHeap = DX_NEW DXUTDeviceSettings;
        ...
        !bKeepCurrentWindowSize )          // only resize if pp.BackbufferWidth/Height were not 0
    {
        ...
        if( bClipWindowToSingleAdapter && !IsIconic(DXUTGetHWNDDeviceWindowed()) )
        {
            ...
            DXUT_GetMonitorInfo( hAdapterMonitor, &miAdapter );
            HMONITOR hWindowMonitor = DXUT_MonitorFromWindow( DXUTGetHWND(), MONITOR_DEFAULTTOPRIMARY );
    ...
    if( bNeedToResize ) 
    {
        ...
        if( bClipWindowToSingleAdapter )
        {
            ...
            MONITORINFO miAdapter;
            miAdapter.cbSize = sizeof(MONITORINFO);
            DXUT_GetMonitorInfo( DXUTGetD3DObject()->GetAdapterMonitor( pNewDeviceSettings->AdapterOrdinal ), &miAdapter );

            // Get the rect of the monitor attached to the window
            MONITORINFO miWindow;
            miWindow.cbSize = sizeof(MONITORINFO);
            DXUT_GetMonitorInfo( DXUT_MonitorFromWindow( DXUTGetHWND(), MONITOR_DEFAULTTOPRIMARY ), &miWindow );
...
//--------------------------------------------------------------------------------------
// Shuts down DirectX without bothering about the window
//--------------------------------------------------------------------------------------
void DXUTSimpleShutdown( )
{
    DXUTCleanup3DEnvironment( true );
...
{ 
    ...
    if( pTimerList == NULL )
    {
        pTimerList = DX_NEW CGrowableArray<DXUT_TIMER>;
        if( pTimerList == NULL )
            return E_OUTOFMEMORY; 
...
void DXUTDisplayErrorMessage( HRESULT hr )
{
    ...
    switch( hr )
    {
        ...
        case DXUTERR_NOCOMPATIBLEDEVICES:    
            nExitCode = 3; 
            if( DXUT_GetSystemMetrics(SM_REMOTESESSION) != 0 )
...
void DXUTSetupCursor()
{
	...
	IDirect3DDevice9* pd3dDevice = DXUTGetD3DDevice();
	if( !DXUTIsWindowed() && GetDXUTState().GetShowCursorWhenFullScreen() && pd3dDevice )
	{
...
HRESULT DXUTSetDeviceCursor( IDirect3DDevice9* pd3dDevice, HCURSOR hCursor, bool bAddWatermark )
{
    ...
    pcrArrayMask = DX_NEW DWORD[dwWidth * dwHeightSrc];
    ...
    if (!bBWCursor)
    {
        pcrArrayColor = DX_NEW DWORD[dwWidth * dwHeightDest];
...
BOOL WINAPI DXUT_GetMonitorInfo(HMONITOR hMonitor, LPMONITORINFO lpMonitorInfo)
{
    if( GetDXUTState().GetFnGetMonitorInfo() ) 
        return GetDXUTState().GetFnGetMonitorInfo()(hMonitor, lpMonitorInfo);
...
int WINAPI DXUT_GetSystemMetrics(int nIndex)
{
    if( GetDXUTState().GetFnGetSystemMetrics() ) 
        return GetDXUTState().GetFnGetSystemMetrics()(nIndex); 
    return GetSystemMetrics(nIndex);
}

HMONITOR WINAPI DXUT_MonitorFromWindow(HWND hWnd, DWORD dwFlags)
{
    if( GetDXUTState().GetFnMonitorFromWindow() ) 
        return GetDXUTState().GetFnMonitorFromWindow()(hWnd, dwFlags);
